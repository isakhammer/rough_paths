...
Recent work combines signature transforms from rough path theory with neural networks to tackle problems in data science (cf. \cite{LZL19}, \cite{WIN19}, \cite{XSJ17}). The central task is to learn the nonlinear relationship between an input data stream (a sequence of points in a vector space) and an output. For instance, atmospheric data from the last 24 hours could be used to predict weather for the next 24 hours.

An input stream is naturally ordered, allowing it to be represented as a continuous path in a vector space \( V \). The signature transform \( S(x) \) maps a path \( x: [0, T] \to V \) to a sequence of global iterated integrals:
\[
S(x) = \left( \int_0^T \! dx_t \otimes dx_t, \, \int_0^{t_1} \cdots \int_0^{t_n} dx_{t_1} \otimes \cdots \otimes dx_{t_n}, \dots \right)
\]
...
\textbf{Property 1.} The truncated signature \( S_N(x) \) converges to the full signature \( S(x) \) rapidly, even for moderate \( N \), ensuring minimal information loss.

\textbf{Property 2.} Polynomial functions on the signature space are linear, as implied by the shuffle product formula (\cite{formula115}). Thus, lifting the problem to the signature space linearizes it, making the solution robust and model-free.
